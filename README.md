...
- On Linux / WSL, Antigravity may persist OAuth tokens through Secret Service / keyring rather than plain files. If `agy` logs mention `org.freedesktop.secrets` or `failed to persist token to keyring`, Shogunate symlinks cannot keep the login by themselves. On Ubuntu / WSL, install `gnome-keyring` and `libsecret-tools`, then start / unlock `gnome-keyring-daemon` and log in with `agy` again.
