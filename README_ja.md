...
- Linux / WSL の Antigravity は、OAuth token をファイルではなく Secret Service / keyring に保存する場合があります。Shogunate は Agy 起動直前に `scripts/ensure_antigravity_keyring.sh` を実行し、`gnome-keyring-daemon` の起動を best-effort で試します。`agy` のログに `failed to persist token to keyring` や `failed to unlock correct collection` が出る環境では、Ubuntu / WSL に `gnome-keyring` と `libsecret-tools` を入れ、host 側で一度 `agy` ログインを完了してください。空パスワード keyring などの作成・切替はユーザーの明示操作に任せ、Shogunate は既存 keyring を削除・再作成しません。初回 Terms / onboarding の再表示を避けるため、host の `.gemini/antigravity-cli/cache/onboarding.json` は role-local home へ未作成時だけ初期コピーします。
