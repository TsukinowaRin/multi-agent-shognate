...
launcher は既定で clean start し、`goza-no-ma` を作成したら起動中 window に先に attach します。Codex などのエージェント CLI は attach 後に裏の `overview` window で起動するため、Codex TUI は通常の手動起動に近い表示端末上で初期化されます。起動完了後は自動で `overview` に切り替わります。ログインが必要な場合は、tmux 上で該当 pane の案内に従ってログインします。既存 state を保つ場合は `./Shogunate-Runtime.sh --resume`、attach しない場合は `./Shogunate-Runtime.sh --no-attach` を使います。
