...
このリポジトリは upstream の README 構成を保ちつつ、運用上の差分だけを追加しています。

- 実 runtime の正本は `goza-no-ma:overview` です。`shogun:main` / `gunshi:main` / `multiagent:agents` は Android 互換の proxy session として併設しています。
- `cgo` は本物の御座の間を開きます。`css` / `csg` / `csm` は live pane へ移動します。
- upstream 以外の追加 CLI として `Gemini CLI` / `OpenCode` / `Kilo` / `localapi`、および `Ollama` / `LM Studio` 連携を持ちます。
- 現在の既定構成は保守寄りで、全役職 `codex`、`model: auto`、初期足軽は `ashigaru1` と `ashigaru2` の2名です。
- 家老は上様の意図から自律的に人員配置を決める前提です。陣形名を明示しなくても、家老が分担・人数・並列度を判断します。

### このフォークで最初に使うコマンド

```bash
bash scripts/configure_agents.sh   # 役職ごとの CLI / model 設定
bash scripts/goza_no_ma.sh         # 御座の間を開く
bash scripts/focus_agent_pane.sh karo
```

upstream Android アプリをそのまま使う場合の設定値は次のとおりです。

- 将軍 session: `shogun`
- エージェント session: `multiagent`
- project path: このリポジトリのルート

---

## これは何？

**multi-agent-shognate** は `multi-agent-shogun` を基に、このリポジトリ用の tmux / Android 運用へ合わせた fork です。複数の AI コーディング CLI インスタンスを同時に実行し、戦国時代の軍制のように統率します。**Claude Code**、**OpenAI Codex**、**GitHub Copilot**、**Kimi Code** の4CLIに対応します。
...
| | Claude Code `Task` ツール | Claude Code Agent Teams | LangGraph | CrewAI | **multi-agent-shognate** |
|---|---|---|---|---|---|
...
このリポジトリの **GitHub Releases** から `multi-agent-shognate-installer.bat` をダウンロードし、インストールしたいフォルダに置く
...
WSL2 / Ubuntu を確認し、Release asset として使う場合は GitHub から**同じ tag のソース**を **`install.bat` を置いたフォルダへ取得**して、**そのまま `first_setup.sh` まで自動実行**します。
...
`install.bat` が standalone 実行時は同じ Release tag のコードを **置かれたフォルダへ取得**し、その後 Ubuntu 側で `first_setup.sh` まで実行します。
...
> **Note:** 現在 Android のみ対応。iOS 版は未対応です。必要なら [Issue](https://github.com/TsukinowaRin/multi-agent-shognate/issues) を作成してください。
...
2. **SSH接続情報を設定**（設定タブ）
   ...
   | 項目 | 入力例 | 説明 |
   |------|--------|------|
   | SSHホスト | `ssh.example.local` | 到達可能なサーバーの IP またはホスト名。初期値は空欄 |
...
スマホから到達できる SSH 接続先を利用者側で用意してください。アプリ側は `SSHホスト` / `SSHポート` / `SSHユーザー` / `プロジェクトパス` が正しく届けば動作します。
...
| 名前 | 一言で言うと | 役割 |
|------|------------|------|
| SSH | ホストへ入るための手段 | tmux が動くマシンに接続する |
| [Termux](https://termux.dev/) | スマホの黒い画面 | Android から SSH を使うための端末 |
...
2. スマホのTermuxから：
   ...
   ssh あなたのユーザー名@あなたの接続先ホスト
   css    # 将軍に繋がる
   ```
3. ＋ボタンで新しいウィンドウを開いて、部下の様子も見る：
   ```sh
   ssh あなたのユーザー名@あなたの接続先ホスト
   csm    # 家老+足軽の9ペインが広がる
   ```
...
3. 必要なら GitHub から同じ Release tag のコードを `install.bat` を置いたフォルダへ取得
...
- ✅ standalone 実行時は同じ Release tag のコードを `install.bat` を置いたフォルダへ取得
