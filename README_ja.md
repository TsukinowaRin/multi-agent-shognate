...
- Linux / WSL の Antigravity は、OAuth token をファイルではなく Secret Service / keyring に保存する場合があります。`agy` のログに `org.freedesktop.secrets` や `failed to persist token to keyring` が出る環境では、Shogunate の symlink だけではログイン保持できません。Ubuntu / WSL では `sudo apt install -y gnome-keyring libsecret-tools` を入れ、`gnome-keyring-daemon` を起動 / unlock してから `agy` で再ログインしてください。
