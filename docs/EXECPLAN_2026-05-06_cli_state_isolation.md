...
- 2026-05-09 の確認により、OpenCode / Kilo も host `auth.json` だけを共有し、SQLite DB / prompt history は初期コピーせず pane-local に保持する方針へ更新した。
- 2026-05-11 の確認により、OpenCode / Kilo の `model.json` は role-local file が未作成のときだけ host から初期コピーし、その後は pane-local に保持する方針へ更新した。
...
- [x] OpenCode / Kilo は host `auth.json` を共有し、model state は初回だけ seed、DB / history state は pane-local に保つ挙動へ更新した。
- [x] unit test を更新した。
- [x] README 英日を更新した。
- [x] 検証結果を記録した。
...
- `opencode` / `kilo` は host `model.json` を初回だけ seed し、host DB / prompt history は初期コピーしない。既存の role-local regular file は残し、古い symlink だけ外す。
