...
- [x] 隔離 project で Shogunate runtime を実AI CLI 付きで起動し、役職 pane / queue / watcher / dashboard の実動作を確認した。実測 session は `shogunate-mod-real-ai-e2e-20260620161129`、daemon は `goza-runtime-shogunate-mod-real-ai-e2e-20260620161129`、target project は `/mnt/d/Git_WorkSpace/shogunate-real-ai-project-real-ai-e2e-20260620161129`。5/5 CLI 起動、5/5 ready、watcher / bridge / daemon 起動を確認した。
...
- [x] Android 実機で APK install / app 起動に加え、可能な範囲で Pair / SSH / dashboard 接続を確認した。実機 `661ecd40` へ `installDebug`、`com.shogun.android/.MainActivity` cold start、app process `22527`、Foreground service 起動、PairingProvider の公開鍵発行を確認した。Pair server は実機生成の公開鍵で `OnePlus-LE2121` を承認し、sandbox `authorized_keys` へ1件登録、`muro@127.0.0.1:2223` を返し、成功後 `Pairing complete` で自動停止した。Android UI の Connect タップから dashboard 表示までの完全自動確認は未実施で、残リスクとして扱う。
