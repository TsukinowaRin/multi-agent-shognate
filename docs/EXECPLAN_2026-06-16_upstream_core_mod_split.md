...
- [x] MOD-owned development command surface に `make mod-check` を追加し、package prepublish gate と detached source runtime smoke を一括で再実行できるようにした。
...
- PASS: `make -n mod-check` and `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_makefile_has_mod_canonical_copy` after adding the combined MOD verification target.
