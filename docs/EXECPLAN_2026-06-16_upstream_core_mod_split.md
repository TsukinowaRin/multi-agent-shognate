...
- [x] 代表 wrapper smoke の必須 entrypoint set を npm CLI / Pair server / shell aliases / agent status に固定し、source checkout / cURL release archive / npm package tarball の3面検証対象が黙って縮小しないようにした。
...
- [x] 実AI CLI availability を確認し、秘密情報を読まずに本物AI応答を取得した。`claude -p --max-budget-usd 0.25 --permission-mode dontAsk --tools "" --no-session-persistence` で `SHOGUNATE_REAL_AI_RECHECK_OK` を確認した。`agy --print` も実応答したが、flag 解釈がこの環境では不安定だったため runtime 実証は Claude を採用した。
- [x] 隔離 project で Shogunate runtime を実AI CLI 付きで起動し、役職 pane / queue / watcher / dashboard の実動作を確認した。実測 session は `shogunate-mod-real-ai-e2e-20260620161129`、daemon は `goza-runtime-shogunate-mod-real-ai-e2e-20260620161129`、target project は `<workspace>/shogunate-real-ai-project-real-ai-e2e-20260620161129`。5/5 CLI 起動、5/5 ready、watcher / bridge / daemon 起動を確認した。
- [x] 実AIへ小タスクを渡し、隔離 project 内の成果物生成を確認した。`ashigaru1` に `real_ai_result.md` 作成を依頼し、1行目 `SHOGUNATE_REAL_RUNTIME_OK`、2行目 target project cwd が生成されたことを確認した。
- [x] Android 実機で APK install / app 起動に加え、Pair / SSH / dashboard 接続を確認した。実機 `661ecd40` へ `installDebug`、`com.shogun.android/.MainActivity` cold start、app process `22527`、Foreground service 起動、PairingProvider の公開鍵発行を確認した。Pair server は実機生成の公開鍵で `OnePlus-LE2121` を承認し、sandbox `authorized_keys` へ1件登録、`<user>@127.0.0.1:2223` を返し、成功後 `Pairing complete` で自動停止した。
- [x] 実AI TUI 検証で見つかった Claude 停止要因を修正した。隔離 role home は host Claude state を directory link で引き継ぎ、nested `.claude/.credentials.json` relink は行わない。runtime 既定は host user settings の破損や権限 prompt に引きずられにくく、かつ tools を無効化しない `--setting-sources local --permission-mode auto` にした。
- [x] Android UI の Connect から dashboard 表示までの実機E2Eを完走した。実測 run は `android-ui-e2e-20260620-181712`、実機は OnePlus `LE2121` serial `661ecd40`。アプリ初期状態から設定タブで `接続` を押し、Pair prompt承認後に画面上で `Pair: OK`、`接続: OK`、`tmux: OK`、`Project: OK`、`将軍 target: OK`、`エージェント target: OK`、`dashboard.md: OK` を確認した。戦況タブでは WebView に `dashboard.md` が描画され、完了後 screenshot `runtime_sandboxes/android-ui-e2e-20260620-181712/dashboard_completed.png` で `cmd_001 ... ✅ 完了` を確認した。
- [x] Android将軍タブから実AI Shogun paneへ命令を送り、Shogun→Karo→Ashigaru の実役職paneを通って target project に成果物が生成されることを確認した。Android入力: `Create file android_ui_e2e_from_app.md with line SHOGUNATE_ANDROID_UI_E2E_OK in the target project.`。成果物 `<workspace>/shogunate-android-ui-e2e-project-android-ui-e2e-20260620-181712/android_ui_e2e_from_app.md` の1行目 `SHOGUNATE_ANDROID_UI_E2E_OK`、Ashigaru report `status: done`、dashboard戦果欄 `cmd_001 ... ✅ 完了` を確認した。
- [x] 実機E2Eで見つかった Pair後UI state不一致を修正した。Pair成功時に ViewModel が返却 host/port/user/key/project/targets を `appliedConnectionSettings` として公開し、SettingsScreen が表示stateへ反映する。これによりPair直後のSSH失敗や再試行時に、古い空user/keyで保存済みPair設定を上書きしない。
- [x] 実機E2Eで見つかった ADB duplicate 問題を修正した。ADBに同じ端末がUSB serial `661ecd40` と Wireless Debugging serial `adb-661ecd40-..._adb-tls-connect._tcp` の2件で見える場合でも、USB候補が1件なら `shogunate_mod/pair/server.py` がそのserialを選んで `adb -s <serial> reverse` を実行する。unit testと実端末短時間起動で `USB reverse: Android 127.0.0.1:18765 -> pair, 127.0.0.1:2224 -> SSH 2223` を確認した。
- [x] 実機E2Eで見つかった Karo の優先順位問題を修正した。`cmd_new` と `audit_action_required` が未読で混在した場合、Karo は advisory audit cleanup より先に user-visible `cmd_new` をdispatchする、と Karo role source / generated instructions / OpenCode agent definitions に明記した。
...
- PASS: direct `diff -q Makefile shogunate_mod/development/Makefile`, `diff -q .github/workflows/package-release.yml shogunate_mod/package/workflows/package-release.yml`, and `diff -q tests/unit/test_package_distribution.py shogunate_mod/tests/unit/test_package_distribution.py` after wiring release package cURL smoke to the prepared archive.
- PASS: YAML parse for `.github/workflows/package-release.yml` and `shogunate_mod/package/workflows/package-release.yml` after wiring release package cURL smoke to the prepared archive.
...
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_release_tar_and_zip_archives_have_same_file_boundary tests.unit.test_package_distribution.PackageDistributionContractTests.test_release_archive_actual_runtime_boundary` after adding the release tar/zip boundary parity contract.
- PASS: direct `diff -q tests/unit/test_package_distribution.py shogunate_mod/tests/unit/test_package_distribution.py` and `git diff --check` after adding the release tar/zip boundary parity contract.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution` ran 131 package distribution contract tests after adding the release tar/zip boundary parity contract.
...
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_public_readmes_have_mod_canonical_copy` after adding the public README fixed/latest cURL contract.
...
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_compatibility_wrappers_execute_mod_sources` after adding direct execution smoke for representative compatibility wrappers.
- PASS: direct `diff -q tests/unit/test_package_distribution.py shogunate_mod/tests/unit/test_package_distribution.py` after adding direct execution smoke for representative compatibility wrappers.
- PASS: `git diff --check` after adding direct execution smoke for representative compatibility wrappers.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution` ran 133 package distribution contract tests after adding direct execution smoke for representative compatibility wrappers.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_release_archive_representative_wrappers_execute_mod_sources` after adding release archive extraction smoke for representative compatibility wrappers.
...
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_compatibility_wrappers_execute_mod_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_release_archive_representative_wrappers_execute_mod_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_npm_package_representative_wrappers_execute_mod_sources` after adding npm CLI entrypoint smoke to source checkout / cURL release archive / npm package tarball surfaces.
- PASS: direct `diff -q tests/unit/test_package_distribution.py shogunate_mod/tests/unit/test_package_distribution.py` after adding npm CLI entrypoint smoke.
- PASS: `git diff --check` after adding npm CLI entrypoint smoke.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution` ran 135 package distribution contract tests after adding npm CLI entrypoint smoke.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_compatibility_wrappers_execute_mod_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_release_archive_representative_wrappers_execute_mod_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_npm_package_representative_wrappers_execute_mod_sources` after sharing the representative wrapper smoke case list across source/archive/npm package surfaces.
...
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_wrapper_smoke_cases_are_manifest_wrappers tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_wrapper_smoke_cases_have_mod_delegates tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_compatibility_wrappers_execute_mod_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_release_archive_representative_wrappers_execute_mod_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_npm_package_representative_wrappers_execute_mod_sources` after requiring representative smoke wrappers to have a single existing MOD delegate.
- PASS: direct `diff -q tests/unit/test_package_distribution.py shogunate_mod/tests/unit/test_package_distribution.py` after requiring representative smoke wrappers to have a single existing MOD delegate.
- PASS: `git diff --check` after requiring representative smoke wrappers to have a single existing MOD delegate.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution` ran 137 package distribution contract tests after requiring representative smoke wrappers to have a single existing MOD delegate.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_wrapper_smoke_cases_cover_required_entrypoints tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_wrapper_smoke_cases_are_manifest_wrappers tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_wrapper_smoke_cases_have_mod_delegates` after requiring the representative smoke set to remain npm CLI / Pair server / shell aliases / agent status.
- PASS: direct `diff -q tests/unit/test_package_distribution.py shogunate_mod/tests/unit/test_package_distribution.py` after adding the representative smoke required-entrypoint contract.
- PASS: `git diff --check` after adding the representative smoke required-entrypoint contract.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution` ran 138 package distribution contract tests after adding the representative smoke required-entrypoint contract.
- PASS: `make package-check` after committing the representative smoke required-entrypoint contract; prepublish source syntax checks, 138 package distribution contract tests, 34 MOD behavior unit tests, and generated instruction freshness all passed.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_npm_wrapper_points_to_curl_bootstrap tests.unit.test_package_distribution.PackageDistributionContractTests.test_npm_cli_run_and_pair_dispatch_to_mod_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_representative_compatibility_wrappers_execute_mod_sources` after changing MOD npm CLI `run` / `pair` dispatch to call MOD canonical sources directly.
- PASS: `node bin/shogunate.js --help && node bin/shogunate.js run --help && node bin/shogunate.js pair --help` after changing MOD npm CLI dispatch.
...
- PASS: direct `diff -q tests/unit/test_package_distribution.py shogunate_mod/tests/unit/test_package_distribution.py` after updating the package distribution contract.
- 実証優先のため、source checkout と package install 後の runtime 起動を実プロセスで確認した。source checkout は detached worktree `runtime_sandboxes/proof-20260619134105` から `shutsujin_departure.sh -s -c` を実行し、tmux session `shogunate-mod-proof-20260619134105` / `goza` window / `shogun,gunkan,karo,gunshi,ashigaru1` pane / `queue/runtime/agent_cli.tsv` を確認した。
- PASS: source runtime smoke `SHOGUNATE_SOURCE_SMOKE_KEEP=1 SHOGUNATE_SOURCE_SMOKE_RUN_ID=proof-20260619134105 bash shogunate_mod/runtime/source_smoke.sh` completed and left evidence in `runtime_sandboxes/proof-20260619134105`.
- cURL/package install 後の `shogunate` command からも setup-only runtime 起動を実証した。local release archive を `shogunate_mod/package/bootstrap.sh` 経由で temp HOME に導入し、`shogunate --project <project> clean --no-attach -s` で tmux session `shogunate-package-proof-proof-package-runtime-20260619134810` / runtime workspace `home/.shogunate/workspaces/project-6a3fca54` / project metadata / `queue/runtime/target_project` / `queue/runtime/engine_dir` / role panes を確認した。
- PASS: Android debug build and unit checks via `make android-check`;実機 `LE2121` / Android 14 へ `adb install -r android/app/build/outputs/apk/debug/app-debug.apk` succeeded, installed version was `versionName=5.2.0.3`, `versionCode=52003`, and `MainActivity` was visible without recent crash logs. Screenshot evidence: `runtime_sandboxes/proof-artifacts/android-main-202606191343.png`.
- PASS: Pair server HTTP flow was verified with temporary `authorized_keys` only. `/health` returned runtime/project metadata, `/pair` accepted a generated public key after local Password approval, returned project-specific tmux targets `shogunate-mod-proof-20260619134105:goza.0` and `shogunate-mod-proof-20260619134105:goza`, wrote one authorized key, and stopped automatically after success. Evidence: `runtime_sandboxes/proof-artifacts/pair-session-target-20260619134601/`.
- PASS: Android app one-touch USB reconnect was verified on実機 `LE2121` by `adb reverse tcp:2222 tcp:2223` and pressing the app Settings screen `USB` / `接続` controls. The app reported `接続: OK`, `tmux: OK`, `Project: OK`, `将軍 target: OK`, and `dashboard.md: OK`, proving the installed app can reach the PC SSH service and inspect the Shogunate runtime from the normal UI path. This first reconnect used the app's previously saved agent target, so it correctly exposed a stale-target failure for `shogunate:goza`.
- PASS: Android app Pair UI flow was then verified on実機 `LE2121` without touching the user's real `~/.ssh/authorized_keys`. The test removed the `tcp:2222` reverse to force first SSH failure, kept `tcp:8765 -> 8765` for Pair and `tcp:2223 -> 2223` for post-Pair SSH, started `shogunate_mod/pair/server.py` with a workspace-local temporary `authorized_keys`, approved device `OnePlus-LE2121`, and observed automatic Pair server shutdown. The app reported `Pair: OK`, `接続: OK`, `tmux: OK`, `Project: OK`, `将軍 target: OK`, `エージェント target: OK`, and `dashboard.md: OK` after saving project-specific targets `shogunate-mod-proof-20260619134105:goza.0` / `shogunate-mod-proof-20260619134105:goza`. Evidence: `runtime_sandboxes/proof-artifacts/android-after-pair-flow-ui.xml`, `runtime_sandboxes/proof-artifacts/android-pair-app-flow/android-after-pair-flow.png`, and workspace-local `runtime_sandboxes/proof-artifacts/android-pair-app-flow/authorized_keys` containing exactly one public key line.
...
- Shell alias installer と `setup.sh` 互換実行もさらに MOD 側へ寄せた。`scripts/install_shell_aliases.sh` は互換入口のまま残すが、`.bashrc` / shell rc へ書き込む source 行は `shogunate_mod/shell/aliases.sh` 正本へ変更した。`shogunate_mod/runtime/setup_compat.sh` は root `shutsujin_departure.sh` wrapper を再呼び出しせず、`shogunate_mod/runtime/load.sh` を直接読み込んで `run_shutsujin_departure` を実行する。
- PASS: `bats tests/unit/test_shell_aliases.bats` after moving the installed shell rc source line to `shogunate_mod/shell/aliases.sh`.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_mod_runtime_helpers_call_mod_canonical_sources tests.unit.test_package_distribution.PackageDistributionContractTests.test_compatibility_wrappers_delegate_to_shogunate_mod` after adding the alias installer / setup compatibility contracts.
- PASS: `bash -n shogunate_mod/shell/install_aliases.sh scripts/install_shell_aliases.sh shogunate_mod/runtime/setup_compat.sh setup.sh`, direct root/MOD test sync checks, and `git diff --check` after the alias installer / setup compatibility changes.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution` ran 145 tests after committing the alias installer / setup compatibility changes, proving the updated files are present in `git archive HEAD` and npm/package surfaces.
- PASS: `make package-check` after committing the alias installer / setup compatibility changes; prepublish source syntax checks, 145 package distribution contract tests, 34 MOD behavior unit tests, generated instruction freshness, and dirty worktree gate all passed.
- Runtime entrypoint も MOD 正本へ寄せた。`shogunate_mod/runtime/entrypoint.sh` を追加し、root `shutsujin_departure.sh` はこの MOD entrypoint を exec する薄い互換入口にした。`shogunate_mod/runtime/runtime_launcher.sh` / `shogunate_mod/runtime/shutsujin_launcher.sh` / `shogunate_mod/runtime/setup_compat.sh` は root wrapper を再呼び出しせず、MOD entrypoint を直接実行する。
- PASS: `bash -n shutsujin_departure.sh shogunate_mod/runtime/entrypoint.sh shogunate_mod/runtime/setup_compat.sh shogunate_mod/runtime/launcher.sh shogunate_mod/runtime/runtime_launcher.sh shogunate_mod/runtime/shutsujin_launcher.sh setup.sh` after adding the MOD runtime entrypoint.
- PASS: `bats tests/unit/test_runtime_launchers.bats` after requiring MOD runtime launchers to call `shogunate_mod/runtime/entrypoint.sh` instead of `bash shutsujin_departure.sh`.
- PASS: `bats tests/unit/test_mux_parity.bats tests/unit/test_runtime_launchers.bats tests/unit/test_mux_parity_smoke.bats` ran 79 tmux / launcher / mux-smoke tests after the entrypoint change.
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_compatibility_wrappers_delegate_to_shogunate_mod tests.unit.test_package_distribution.PackageDistributionContractTests.test_manifest_compatibility_wrapper_targets_are_canonical_paths tests.unit.test_package_distribution.PackageDistributionContractTests.test_npm_wrapper_points_to_curl_bootstrap` after declaring the entrypoint in `shogunate_mod/manifest.yaml`.
...
- Agent-facing instruction の mailbox system label / command table / event-driven examples / nudge mechanism wording も root wrapper 由来の `inbox_write.sh` / bare `inbox_watcher.sh` 表記から MOD 正本 `shogunate_mod/inbox/write.sh` / `shogunate_mod/watcher/inbox_watcher.sh` 表記へ寄せた。説明文は `mailbox write ... via shogunate_mod/inbox/write.sh` とし、互換 wrapper 名を agent が正本として覚えないようにした。
- PASS: `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_instruction_runtime_commands_do_not_point_to_root_wrappers tests.unit.test_package_distribution.PackageDistributionContractTests.test_autoload_mailbox_protocol_uses_mod_canonical_writer` after extending the instruction command contract to forbid legacy mailbox labels and require the MOD mailbox/watcher wording.
- PASS: `rg -n '## Mailbox System \(inbox_write\.sh\)|\| inbox_write\.sh \||No sleep needed\. All messages guaranteed delivered by inbox_watcher\.sh|\*\*Why no background monitor\*\*: inbox_watcher\.sh|- inbox_watcher\.sh sends|scripts/(inbox_write|ntfy|karo_done_to_shogun_bridge_daemon|gunkan_codd_audit|gunkan_emergency_stop|localapi_repl)\.(sh|py)|lib/file_watch\.sh' shogunate_mod/instructions/autoload shogunate_mod/instructions/source instructions AGENTS.md CLAUDE.md .github/copilot-instructions.md agents/default/system.md .opencode/agents -g '*.md' || true` returned no matches after regenerating instruction outputs.
- PASS: `bash shogunate_mod/instructions/ensure_generated.sh`, `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution`, root/MOD package distribution test sync, and `git diff --check` after the mailbox label cleanup.
- MOD view helper の help / alias guidance も root `scripts/goza_no_ma.sh` / `scripts/focus_agent_pane.sh` 互換入口ではなく `shogunate_mod/view/goza_no_ma.sh` / `shogunate_mod/view/focus_agent_pane.sh` 正本を案内するようにした。旧 `scripts/*` は wrapper として残す。
- PASS: `bash -n shogunate_mod/view/goza_no_ma.sh shogunate_mod/view/focus_agent_pane.sh` and `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.unit.test_package_distribution.PackageDistributionContractTests.test_mod_runtime_helpers_call_mod_canonical_sources` after adding MOD view helpers to the canonical-source contract.
- PASS: `bats tests/unit/test_mux_parity.bats --timing` ran 72 mux / Goza / focus / watcher tests after updating MOD view helper guidance, followed by `git diff --check`.
- MOD 正本 helper 自身の Usage / header / example も root `scripts/*` / `lib/*` 互換入口から MOD 正本 path へ寄せた。対象は inbox writer / inbox watcher / Gunkan emergency stop / CoDD check / rate-limit status / agent status command+library / update stop-apply / mux parity smoke。legacy process cleanup のための `$SCRIPT_DIR/scripts/...` 検出は互換目的として残した。
- PASS: `bash -n shogunate_mod/inbox/write.sh shogunate_mod/watcher/inbox_watcher.sh shogunate_mod/gunkan/emergency_stop.sh shogunate_mod/gunkan/codd_check.sh shogunate_mod/status/ratelimit_check.sh shogunate_mod/status/command.sh shogunate_mod/status/agent_status.sh shogunate_mod/update/stop_and_apply_update.sh shogunate_mod/runtime/mux_parity_smoke.sh` and targeted `PackageDistributionContractTests.test_mod_runtime_helpers_call_mod_canonical_sources` after adding these helpers to the canonical-source contract.
...
- Windows launcher 正本も root shell wrappers 経由から MOD runtime launchers 直呼びへ寄せた。root `Shogunate-Runtime.bat` / `Shutsujin.bat` は互換 wrapper として残し、`shogunate_mod/windows/runtime_launcher.bat` は WSL 内で `shogunate_mod/runtime/runtime_launcher.sh` を、`shogunate_mod/windows/shutsujin_launcher.bat` は `shogunate_mod/runtime/shutsujin_launcher.sh` を直接実行する。clean / resume debug launcher も root `Shutsujin.bat` ではなく MOD `shogunate_mod/windows/shutsujin_launcher.bat` を呼ぶ。
