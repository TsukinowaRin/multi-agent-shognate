...
- `shogunate pair` は Android app の初回 setup 用。既定で USB reverse を自動試行しつつ Tailscale/LAN でも待ち受ける。`shogunate pair --usb` は廃止し、package command は `shogunate pair` に統一する。Android app は app 内秘密鍵を保持し、PC 側は端末名確認後の Pair Password prompt 承認で公開鍵だけを `~/.ssh/authorized_keys` へ追加する。成功後は PC 側が best-effort で Shogunate runtime を起動し、以後は登録済み SSH 鍵で直接接続する。SSH port は TCP open だけでなく SSH banner で検出し、壊れた Windows portproxy / stale forwarding を避ける。USB は `127.0.0.1`、無線は app が入力した PC address を使い、Pair terminal は app へ返した `user@host:port` を表示する。
