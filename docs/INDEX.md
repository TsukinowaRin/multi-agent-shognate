...
- Android app の将軍タブ既定 target は `agent:shogun`。これは実 tmux target ではなく、`@agent_id=shogun` の pane を自動検出する仮想 target。設定画面の通常導線は `ワンタッチ接続` で、従来の SSH 詳細入力は `マニュアルモード` 配下。通常画面の `接続先` 欄は DNS 名、URL、Tailscale IP、LAN IP を受け取り、SSH 用 host/port へ正規化する。URL path/query/fragment は無視する。USB 接続は `android/tools/setup_android_ssh.sh --usb` で host SSH port を自動検出し、`adb reverse tcp:2222 tcp:<detected>` を張る。Android 側は常に `127.0.0.1:2222` を使う。`--pair` / `--pair-usb` は Android app 内生成鍵の公開鍵だけをPCへ登録し、秘密鍵をスマホ内に残したまま鍵認証つき setup intent を送る。`--pair-wireless` は初回USBデバッグで同じ鍵を登録し、Tailscale/LAN へ直接接続する設定を送る。接続先の固定は `--host <DNS-or-URL-or-IP>` または `SHOGUNATE_PAIR_HOST=<DNS-or-URL-or-IP>`。無線手動は `--wireless` が候補 IP ごとの接続設定リンク（内部形式 `shogunate://setup...`）と optional QR を出し、Android app 設定画面の `設定取込` で保存できる。
- Android APK の `versionName` は Shogunate 本体バージョン + fork/app 改訂番号にする。例: 本体 `5.1.0` の Android 側1回目の改訂は `5.1.0.1`。`versionCode` は同じ更新順に単調増加させる。
