...
### 要求
1. Shogunate launcher の通常起動では、`goza-no-ma` 作成後に起動中 window へ先に tmux attach し、Codex などのエージェント CLI は裏の `overview` window で attach 済み client 上に起動すること。
...
### 受け入れ条件（観測可能）
1. コマンド: `bash -n lib/cli_adapter.sh && bats tests/unit/test_cli_adapter.bats`
   - 期待結果: executable resolver が HOME 配下 native CLI を優先し、OpenCode / Kilo の host auth link と DB / model / history symlink cleanup が PASS する。
2. コマンド: `source lib/cli_adapter.sh; build_cli_command shogun`
...
5. OpenCode / Kilo は CLI 内で provider / login を選べない場合があるため、host 側の auth file だけ role-local HOME へ symlink して使うこと。
...
2. コマンド: `bats tests/unit/test_cli_adapter.bats tests/unit/test_sync_opencode_config.bats`
   - 期待結果: Gemini custom command でも `--yolo` が補完され、OpenCode / Kilo は host 側 auth file を symlink し、OpenCode / Kilo 用 `opencode.json` に `permission: allow` が出力される。
...
2. `claude` / `copilot` / `kimi` / `gemini` は既存の bypass 系起動フラグを維持し、`codex` は通常 UI に近い `--sandbox danger-full-access --ask-for-approval never` で承認不要の full access とすること。
...
1. コマンド: `bats tests/unit/test_cli_adapter.bats tests/unit/test_sync_opencode_config.bats`
   - 期待結果: 既存 CLI の unattended 起動と、`opencode.json` の `permission: allow` 出力が PASS する。
2. コマンド: `rg -n "dangerously-skip-permissions|--sandbox danger-full-access --ask-for-approval never|--yolo|permission: allow" README.md README_ja.md lib/cli_adapter.sh scripts/sync_opencode_config.py`
