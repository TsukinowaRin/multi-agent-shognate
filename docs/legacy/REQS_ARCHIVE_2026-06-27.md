...
## 現在の依頼（2026-08-03: 任意役職のMoA化）

### 目的

Shogunateコマンドから、任意の役職を常設または現在の任務だけのMoA（複数AIによる1役職）として設定・展開できるようにする。MoAには代表者を必須とし、AGMSG経由で複数メンバーが実際に参加して正式成果物へ集約できることを検証する。

### 受け入れ条件

1. 役職ごとに`single`またはデフォルト`moa`を設定でき、代表者、メンバー、定足数、終了条件を検証できる。
2. Shogunateコマンドから現在の任務だけの一時MoAを展開・確認・解散でき、一時指定が役職のデフォルト設定より優先される。
3. 外部からは1つの役職として扱い、正式な命令・報告・判定を出せるのは代表者だけである。
4. AGMSGは通知と成果物ポインタの配送だけに使い、task/report/receiptの正本はShogunate管理下に置く。
5. task ID、role generation、member identity、provenanceが一致しない提案、重複メンバー、定足数不足を完了扱いにしない。
6. 決定的なunit/CLI/security testと、利用可能な実AGMSG環境でのruntime試験がPASSする。環境依存で実行不能な項目は、原因と未検証範囲を明示する。

### 制約と仮定

- 既存の軍議にあるリーダー責任制、Gunkan監査、Ashigaruへの引き渡しを弱めない。
- `AGMSG`のDBや設定は直接操作せず、公式付属scriptだけを使う。
- secret、credential、既存project、既存tmux sessionを変更しない。
- 初期実装は役職共通の展開・集約基盤を作り、役職ごとの意思決定policyは既存責務を保てる最小セットに限定する。
- 2026-08-03追補: npmは配布手段としてだけでなく資材・runtime依存も完全廃止する。cURL bootstrapとcURL release archiveを唯一の導入経路にし、`shogunate`コマンドはNode/npmを要求しない実装へ移行する。
