...
setup() {
  ...
  mkdir -p "$TEST_TMP/scripts" "$TEST_TMP/lib" "$TEST_TMP/config" "$TEST_TMP/queue/runtime" "$TEST_TMP/shogunate_mod/configure" "$TEST_TMP/shogunate_mod/topology"
  cp "$PROJECT_ROOT/scripts/configure_agents.sh" "$TEST_TMP/scripts/configure_agents.sh"
  cp "$PROJECT_ROOT/shogunate_mod/configure/agents.sh" "$TEST_TMP/shogunate_mod/configure/agents.sh"
  cp "$PROJECT_ROOT/lib/topology_adapter.sh" "$TEST_TMP/lib/topology_adapter.sh"
  cp "$PROJECT_ROOT/shogunate_mod/topology/adapter.sh" "$TEST_TMP/shogunate_mod/topology/adapter.sh"
  chmod +x "$TEST_TMP/scripts/configure_agents.sh"
  chmod +x "$TEST_TMP/shogunate_mod/configure/agents.sh"
