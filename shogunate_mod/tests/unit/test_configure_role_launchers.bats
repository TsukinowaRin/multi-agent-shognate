...
@test "configure role launchers: Unix wrappers are valid shell and call canonical script" {
  ...
  run bash -n "$PROJECT_ROOT/shogunate_mod/configure/role_launcher.sh"
  [ "$status" -eq 0 ]

  run grep -F "shogunate_mod/configure/role_launcher.sh" "$PROJECT_ROOT/Shogunate-Configure-Roles.sh"
  [ "$status" -eq 0 ]

  run grep -F "SHOGUNATE_ROLE_CONFIG_PAUSE=1" "$PROJECT_ROOT/Shogunate-Configure-Roles.command"
  [ "$status" -eq 0 ]

  run grep -F "shogunate_mod/configure/runtime_roles.py" "$PROJECT_ROOT/shogunate_mod/configure/role_launcher.sh"
  [ "$status" -eq 0 ]
}
