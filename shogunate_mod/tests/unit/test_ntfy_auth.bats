...
@test "T-AUTH-006b: ntfy_get_auth_args default path resolves project config through wrapper" {
    local mock_dir="$TEST_TMPDIR/project"
    mkdir -p "$mock_dir/config" "$mock_dir/lib" "$mock_dir/shogunate_mod/notify"
    cp "$PROJECT_ROOT/lib/ntfy_auth.sh" "$mock_dir/lib/"
    cp "$PROJECT_ROOT/shogunate_mod/notify/ntfy_auth.sh" "$mock_dir/shogunate_mod/notify/"
    cat > "$mock_dir/config/ntfy_auth.env" << 'EOF'
NTFY_TOKEN=tk_default_path_test
EOF

    run bash -lc "source '$mock_dir/lib/ntfy_auth.sh'; ntfy_get_auth_args"
    [ "$status" -eq 0 ]
    echo "$output" | grep -q 'Bearer tk_default_path_test'
}
...
@test "T-AUTH-011: ntfy.sh includes auth header in curl when token configured" {
    # テスト用のモック環境を構築
    local mock_dir="$TEST_TMPDIR/project"
    mkdir -p "$mock_dir/config" "$mock_dir/scripts" "$mock_dir/lib" "$mock_dir/shogunate_mod/notify"
...
MOCK
    ...
    cp "$mock_dir/mock_curl" "$mock_dir/curl"

    sed "s|^SCRIPT_DIR=.*|SCRIPT_DIR=\"$mock_dir\"|" \
        "$PROJECT_ROOT/shogunate_mod/notify/send.sh" \
        > "$mock_dir/scripts/ntfy_test.sh"
    chmod +x "$mock_dir/scripts/ntfy_test.sh"

    run env PATH="$mock_dir:$PATH" bash "$mock_dir/scripts/ntfy_test.sh" "hello"
    [ "$status" -eq 0 ]
