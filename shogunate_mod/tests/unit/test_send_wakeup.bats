...
@test "T-CODEX-015f: watcher は unread が無くても idle loop で Codex runtime prompt を掃除する" {
    run bash -c '
        grep -q "maintain_codex_runtime_prompt" "'"$PROJECT_ROOT"'/shogunate_mod/watcher/inbox_watcher.sh" &&
        grep -q "dismiss_codex_rate_limit_prompt_if_present" "'"$PROJECT_ROOT"'/shogunate_mod/watcher/inbox_watcher.sh" &&
        grep -q "codex_switch_confirm_prompt_detected" "'"$PROJECT_ROOT"'/shogunate_mod/watcher/inbox_watcher.sh" &&
        grep -q "process_unread_once" "'"$PROJECT_ROOT"'/shogunate_mod/watcher/inbox_watcher.sh" &&
        grep -q "while true; do" "'"$PROJECT_ROOT"'/shogunate_mod/watcher/inbox_watcher.sh"
    '
    [ "$status" -eq 0 ]
}
