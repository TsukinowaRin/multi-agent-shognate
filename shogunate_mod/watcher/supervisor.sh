...
if [ -f "$SCRIPT_DIR/shogunate_mod/watcher/file_watch.sh" ]; then
    # shellcheck source=/dev/null
    source "$SCRIPT_DIR/shogunate_mod/watcher/file_watch.sh"
else
    file_watch_backend_available() { return 0; }
    file_watch_backend() { printf 'polling\n'; }
fi

if [ -f "$SCRIPT_DIR/shogunate_mod/topology/agent_registry.sh" ]; then
    # shellcheck source=/dev/null
    source "$SCRIPT_DIR/shogunate_mod/topology/agent_registry.sh"
fi
