...
@test "shell_aliases は source で repo-local alias を定義する" {
    ...
    [[ "$output" == *"alias css='bash $PROJECT_ROOT/shogunate_mod/view/focus_agent_pane.sh shogun'"* ]]
    [[ "$output" == *"alias cgn='bash $PROJECT_ROOT/shogunate_mod/view/focus_agent_pane.sh gunkan'"* ]]
    [[ "$output" == *"alias csg='bash $PROJECT_ROOT/shogunate_mod/view/focus_agent_pane.sh gunshi'"* ]]
...
@test "shell_aliases は workspace session_name を環境変数へ反映する" {
    ...
    mkdir -p "$root/scripts" "$root/shogunate_mod/shell" "$root/queue/runtime"
    cp "$PROJECT_ROOT/scripts/shell_aliases.sh" "$root/scripts/shell_aliases.sh"
    cp "$PROJECT_ROOT/shogunate_mod/shell/aliases.sh" "$root/shogunate_mod/shell/aliases.sh"
    printf 'shogunate-demo-1234\n' > "$root/queue/runtime/session_name"

    run bash -lc "unset SHOGUNATE_SESSION_NAME GOZA_SESSION_NAME; source '$root/scripts/shell_aliases.sh'; printf '%s %s\n' \"\$SHOGUNATE_SESSION_NAME\" \"\$GOZA_SESSION_NAME\""
    [ "$status" -eq 0 ]
    [ "$output" = "shogunate-demo-1234 shogunate-demo-1234" ]
}
