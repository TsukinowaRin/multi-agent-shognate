...
@test "shell_aliases は source で repo-local alias を定義する" {
    run bash -lc "source '$PROJECT_ROOT/scripts/shell_aliases.sh'; alias cgo; alias css; alias csg; alias csm; alias csst; alias csa; alias cma; alias CGO; alias CSS; alias CSG; alias CSM; alias CSST; alias CSA; alias CMA"
    [ "$status" -eq 0 ]
